...
function Folder(props) {
  ...
  const [fetchFolderRef, setFetchFolderRef] = useState(0);
  ...
  const onDrop = useCallback((acceptedFiles) => {
    ...
    acceptedFiles.forEach((element) => {
      ...
      axiosInstance
        .post(`folder/${params.folderId}/create-file`, formData, formConfig)
        .then((r) => {
          console.log(r.data);
        })
        .catch((e) => {
          ...
          <section className="flex border-t flex-wrap">
            ...
            <div className="w-full md:w-2/3 xl:w-3/4 px-2 md:p-8">
              <h3 className="font-bold text-lg mb-6">
                Folder Contents ({filesInFolder.length})
              </h3>

              <FileList
                files={filesInFolder}
                setFetchFolderRef={setFetchFolderRef}
              />

              <button
                {...getRootProps({ className: "dropzone" })}
                className="my-4 border border-dashed dashed p-8 rounded-xl w-full overflow-hidden"
              >
                <input {...getInputProps()} className="bg-red-500 p-8 block" />
                <p>Drag 'n' drop some files here, or click to select files</p>

                {acceptedFileItems.length > 0 ? (
                  <div className="border-t border-dashed -mb-8 -mx-8 rounded-b-xl px-8  mt-8">
