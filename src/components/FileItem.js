...
export default function FileItem({ file }) {
    ...
    return <div className="block bg-white  p-4 rounded-xl border border-gray-100">
        ...
        <div className="flex">
            ...
            <div className="ml-6"> <span className="block font-semibold my-2">{file.name}</span>

                {file.file ? <Button href={file.file} target="_blank" color="secondary" size="small" className="inline-flex">Open in new tab</Button> : null}
            </div>
        </div>
    </div>
