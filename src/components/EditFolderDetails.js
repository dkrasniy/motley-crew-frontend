...
export default function EditFolderDetails({
  folderId,
  folderData,
  config,
  setCurrentFolder,
  ...
  const [description, setDescription] = useState(
    folderData.description ? folderData.description : ""
  );
  const [desiredCompletionDate, setDesiredCompletionDate] = useState(
    folderData.desiredCompletionDate ? folderData.desiredCompletionDate : ""
  );
  const [expedited, setExpedited] = useState(
    folderData.expedited ? folderData.expedited : false
  );
  const [confidential, setConfidential] = useState(
    folderData.confidential ? folderData.confidential : false
  );
  ...
  const handleSubmit = (e) => {
    ...
    axiosInstance
      .put(
        `folder/${folderId}`,
        { name, description, desiredCompletionDate, expedited, confidential },
        config
      )
      .then((r) => {
        //console.log(r);
          ...
          <div className="flex items-center justify-center min-h-screen">
            ...
            >
              <form
                ...
                className="relative bg-white rounded max-w-xl mx-auto p-8 rounded-xl"
              >
                <Dialog.Title className={"font-bold text-lg"}>
                  Edit Folder Id {folderId}
                </Dialog.Title>
                <Dialog.Description>
                  Edit the folder by updating the fields below Edit the folder
                  by updating the fields below Edit the folder by updating the
                  fields below
                </Dialog.Description>
